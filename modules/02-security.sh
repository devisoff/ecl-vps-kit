...
if ! confirm "Модуль сбросит текущие правила UFW и применит новые. Продолжить?"; then
  warn "Отменено"
  exit 0
fi

node_port="$(prompt_port "Порт ноды, который нужно открыть только для панели" "2222")"
while true; do
  panel_ip="$(prompt_required "IP или CIDR панели")"
  if is_ipv4_or_cidr "${panel_ip}"; then
    break
  fi
  warn "Введите IPv4 или CIDR. Пример: 1.2.3.4 или 1.2.3.0/24"
done
ssh_ports="$(prompt_default "SSH-порты для Fail2Ban" "22,2222")"
...
apt-get update -y
apt-get install -y ufw curl ca-certificates fail2ban iptables
...
# Allow detected SSH ports as an extra lockout protection.
...
if confirm "Запустить установщик TrafficGuard-auto из внешнего репозитория?"; then
