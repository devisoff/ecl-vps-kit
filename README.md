...
- Обновление Ubuntu и установка минимальных базовых пакетов для пунктов 1-2-3.
...
Пункт `0) Обновить Ubuntu и базовые пакеты` выполняет обновление системы и ставит только минимальный набор, необходимый для пунктов `1`, `2` и `3`:

```text
ca-certificates, curl, wget, tar, gzip, procps, iproute2, kmod,
ufw, fail2ban, iptables
```

Зависимости для опциональных пунктов `4) Шейпер трафика` и `5) Установка z4r` устанавливаются только при запуске этих пунктов.
...
```text
1) Создать / изменить правило 443, 5/5 МБ/с
...
При первом запуске пункт 4 сам доустанавливает зависимости шейпера. Пункт 0 их не ставит, потому что шейпер является опциональным модулем. Если отдельному модулю нужна дополнительная зависимость, она устанавливается внутри этого модуля.

В быстром создании правила Enter применяет значения по умолчанию:
...
- Load average.
- RAM и диск.
- Сетевой интерфейс и текущая скорость трафика ↓/↑ в Мбит/с.
- Статус Docker.
...
В разделе `4) Шейпер трафика` доступны два сценария:

```text
0) Авто-правило: #1, режим 1, порт 443, 5/5 МБ/с
1) Создать / изменить правило вручную
```

Пункт `0` применяет правило сразу без ручного ввода. Пункт `1` открывает ручной мастер Reshala Shaper.
